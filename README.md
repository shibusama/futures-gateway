...
- **网关**：Python + `vnpy_ctp`（官方 DLL 绑定），负责登录 SimNow、订阅行情、下单/撤单、回报推送
...
│   ├── ctp.py           # vnpy_ctp 封装（行情 + 交易，一账号一实例）
...
1. **注册 SimNow 仿真账号**：https://www.simnow.com.cn/ （手机号注册 → 实名 → 获得资金账号 + 密码；BrokerID 固定 `9999`）
2. **安装 Python 3.10+**（Windows）
3. **配置账号**：编辑 `config.json`，把 `user_id` / `password` 换成你的 SimNow 凭据；可配置多个账号（多账户）
4. **启动**：双击 `start.bat`（首次会自动建虚拟环境并 `pip install`）
  ...
  "accounts": [
    {
      ...
      "trade_front": "tcp://180.168.146.187:10101",  // 7x24 交易前置（以官网手册为准）
      "md_front": "tcp://180.168.146.187:10111"      // 7x24 行情前置
    }
  ]
}
```

> ⚠️ 前置地址为调研所得，SimNow 官网手册可能有更新，首次连接失败请到 https://www.simnow.com.cn/ 核对最新地址。
...
- Python 3.10+
- `vnpy_ctp`（pip install，自带官方 CTP DLL）
